...
map<string,int> all_weights;

void calcAllW(const string &s)
{
	int w = weights[s];
...
bool balance(const string &s)
{
	if (subs[s].size() == 0) 
	{
		// cout << "root hit at " << s << endl;
		return false;
	}
	
	int w_ideal = all_weights[subs[s][0]];
	
	for (string &n : subs[s])
	{
		if (all_weights[n] != w_ideal)
		{
			// cout << "recurse " << w_ideal << " vs " << all_weights[n] << " _ " << weights[n] << endl;
			if (balance(n))
			{
				cout << weights[n] + w_ideal - all_weights[n] << endl;
			}
